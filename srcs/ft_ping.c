...
void	inthandler(int sig)
{
	...
	{
		printf("\n--- %s ping statistics ---\n", g_all.addr);
        if ((float)(g_all.packet_sent - g_all.packet_receive) * 100 / g_all.packet_sent != NAN)
            printf("%d packets transmitted, %d received, %.1f%% packet loss\n", g_all.packet_sent + 1, g_all.packet_receive, (float)(g_all.packet_sent - g_all.packet_receive) * 100 / g_all.packet_sent);
        else    
            printf("%d packets transmitted, %d received, %.1f%% packet loss\n", g_all.packet_sent + 1, g_all.packet_receive, (float)100);
...
void    send_ping(int sockfd)
{
    ...
    while (1)
    {
        ...
        if (bytes_sent < 0) {
            perror("sendto");
            if (g_all.packet_sent == -1)
                printf("PING %s (%s) %d bytes of data\n", g_all.addr, g_all.addr, PACKET_SIZE);
            printf("ping: sendto: Host is down\n");
        }
        if (g_all.packet_sent == -1 && bytes_sent > 0)
        ...
        usleep(1000000);
        if (g_all.packet_sent == g_all.c_flag)
            inthandler(2);
    }
    return ;
}
